...
import { sendEmail, replacePlaceholders, randomDelay, sleep } from '@/lib/email'
...
export async function POST(req: NextRequest) {
  ...
  // If scheduled, just update campaign and return
  if (scheduleAt) {
  ...
  if (!pending.length) return NextResponse.json({ success: true, sentCount: 0, message: 'No pending recipients' })
